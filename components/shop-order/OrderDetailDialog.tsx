...
function AttachmentPreview({ order }: { order: ShopOrder }) {
  ...
  if (!order.fileUrl) return <span>—</span>;

  return <div className="flex items-center gap-3">
    {thumbnailFailed
      ? <div className="grid h-20 w-20 shrink-0 place-items-center rounded-lg bg-slate-100 px-2 text-center text-xs font-bold text-slate-500">ไม่พบรูปตัวอย่าง</div>
...
export function OrderDetailDialog({ order, pending, onClose, onEdit, onDelete }: { order: ShopOrder; pending: boolean; onClose: () => void; onEdit: () => void; onDelete: () => void }) {
  ...
  const rows = [['ลำดับ', order.no], ['จาก', order.from], ['ถึง', order.to], ['เลขที่', order.number], ['วันที่รับ', formatThaiDate(order.dateIn) || '—'], ['เรื่อง', order.subject], ['หน่วยงานรับ', order.receivingUnit || '—'], ['ผู้รับ', order.receiverName || '—'], ['วันที่ออก', formatThaiDate(order.dateOut) || '—'], ['หมายเหตุ', order.note || '—']];
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-3">
    <section role="dialog" aria-modal="true" aria-labelledby="order-detail-title" onKeyDown={(e) => { if (e.key === 'Escape' && !pending) onClose(); }} className="max-h-[95vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5">
      ...
      <dl className="mt-5 grid gap-x-4 sm:grid-cols-2">{rows.map(([label, value]) => <div key={String(label)} className="border-b border-slate-100 py-2"><dt className="text-xs font-bold text-slate-500">{label}</dt><dd className="mt-1 break-words text-sm">{value}</dd></div>)}<div className="border-b border-slate-100 py-2 sm:col-span-2"><dt className="text-xs font-bold text-slate-500">ไฟล์</dt><dd className="mt-1 text-sm"><AttachmentPreview key={`${order.no}:${order.fileUrl}`} order={order} /></dd></div></dl>
      ...
      {confirming && <div role="alertdialog" aria-modal="true" aria-label="ยืนยันลบรายการ" className="mt-4 rounded-xl bg-rose-50 p-4"><p className="font-bold text-rose-900">ยืนยันลบรายการ #{order.no}?</p><p className="mt-1 text-sm text-rose-800">ไฟล์แนบที่ระบบ OAuth จัดการจะถูกตั้งเวลาย้ายเข้าถังขยะ Google Drive หลัง 30 วัน ส่วนไฟล์เดิม (Legacy) จะไม่ถูกจัดการ</p><div className="mt-3 flex justify-end gap-2"><button onClick={() => setConfirming(false)} className="rounded-lg border px-3 py-1.5">ยกเลิก</button><button disabled={pending} onClick={onDelete} className="rounded-lg bg-rose-700 px-3 py-1.5 font-bold text-white">ยืนยันลบ</button></div></div>}
    </section>
  </div>;
}
